...
def setSellVariables(position_info, TOTAL_PROFIT, BUY_IMBALANCE, current_imbalance):
    # now store variables
    PROFIT = position_info.realized_pl
    SELL_PRICE = position_info.market_value / position_info.qty
...
def setBuyVariables(position_info, current_imbalance):
    PROFIT = response.unrealized_pl
    BUY_PRICE = (position_info.market_value / position_info.qty) - PROFIT
...
# trade variables
BUY_THRESHOLD = 0.6 # how big the imbalance magnitude needs to be to make a trade
SELL_THRESHOLD = 0.4
BUY_PRICE = 0
BUY_IMBALANCE = 0
SELL_PRICE = 0
...
MAX_LOSS = 50
CAN_BUY_TODAY = True
DELAY = 5 # delay in seconds between iterations - DON'T SET BELOW 0.5 OR IT EXCEEDS THE 120 CALLS PER MIN THRESHOLD ON TDAMERITRADE
    ...
    while (True):
        ...
        while(((CURRENT_HOUR + (CURRENT_MIN / 60)) >= 9.5) & (CURRENT_HOUR < 16) & (datetime.today().weekday() != 5) & (datetime.today().weekday() != 6)):
            ...
            if CAN_BUY_TODAY:
                ...
                if imb[1]:
                    current_price = imb[1]
                current_imbalance = (100 - calculate_rsi.calculateRSI(f1, '4h', '1m')) / 100
                print('un-adj imbalance: ' + str(current_imbalance))

                # calculate imbalance and current price
                r"""
                ask = quote['askSize']
                bid = quote['bidSize']
                current_imbalance = (bid - ask) / (bid + ask)
                current_price = quote['lastPrice']
                """ 
                
                TRADE_ORDER_STATUS = checkOrderStatus(api)
                positions = api.list_positions()
                orders = api.list_orders()
                if TRADE_ORDER_STATUS == 1:
                    print('Currently we have one pending BUY order')
                elif TRADE_ORDER_STATUS == -1:
                    print('Currently we have one pending SELL order')
                elif TRADE_ORDER_STATUS != 0:
                    print('ERROR: ORDER STATUS:')
                    
                if PREV_TRADE_ORDER_STATUS != TRADE_ORDER_STATUS: # order updated
                    if (PREV_TRADE_ORDER_STATUS == 1) & (TRADE_ORDER_STATUS == 0): # went from pending buy order to no pending orders - just bought position!
                        OWN_STOCK, PROFIT, BUY_IMBALANCE = setBuyVariables(api.get_position(symbol=f1), current_imbalance)
                    elif (PREV_TRADE_ORDER_STATUS == -1) & (TRADE_ORDER_STATUS == 0): # went from pending seel to no pending orders = just sold position!
                ...
                if (CURRENT_HOUR == 15) & (CURRENT_MIN == 59): # last minute of trading day -> sell all positions
                    if (OWN_STOCK):
                        ...
                        response = placeSellAlpacaOrder(api, f1, AMOUNT_TO_TRADE, current_price, False)
                ...
                print('ticker: ' + str(f1) + ', RSI: ' + str(round(100 - (100 * current_imbalance), 3)) + ', price: $' + str(current_price) + string + ', total profit: $' + str(TOTAL_PROFIT))
                if (current_imbalance >= BUY_THRESHOLD) & (not OWN_STOCK): # stock will probably go up -> buy
                    if (not OWN_STOCK):
                        if (AMOUNT_TO_TRADE * current_price > MAX_MONEY_TO_SPEND): # about to over buy
                            ...
                            response = placeBuyAlpacaOrder(api, f1, AMOUNT_TO_TRADE, current_price, orders, positions)
                    ...
                    response = placeSellAlpacaOrder(api, f1, AMOUNT_TO_TRADE, current_price, False, positions)
                ...
                if (TOTAL_PROFIT <= (MAX_LOSS * -1)) & OWN_STOCK: # lost too much today :(
                    # sell now
                    # placeSellMarketOrder(client,  f1, AMOUNT_TO_TRADE)
                    response = placeSellAlpacaOrder(api, f1, AMOUNT_TO_TRADE, current_price, True, positions)
